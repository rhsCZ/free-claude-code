...
@pytest.mark.asyncio
async def test_global_stop_reports_work_visible_only_in_another_chat(
    handler, mock_platform, incoming_message_factory
) -> None:
    incoming = incoming_message_factory(text="/stop")
    other_scope = MessageScope(platform=incoming.platform, chat_id="other_chat")
    handler.stop_all_tasks = AsyncMock(
        return_value=_stop_outcome(2, scopes=frozenset({other_scope}))
    )
